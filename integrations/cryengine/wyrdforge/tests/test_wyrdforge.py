"""
test_wyrdforge.py — Python mirror tests for WyrdForge CryEngine plugin.

Tests the pure-logic functions from WyrdHelpers.cpp without requiring
a CryEngine build environment or runtime.
"""
...
# ---------------------------------------------------------------------------
# Python mirrors — identical logic to WyrdHelpers.cpp
# ---------------------------------------------------------------------------
...
class TestEscapeJson:
    ...
    def test_control(self):     assert escape_json('\x03') == '\\u0003'
...
class TestNormalizePersonaId:
    ...
    def test_leading_under(self):       assert normalize_persona_id("_Sigrid") == "sigrid"
    def test_trailing_under(self):      assert normalize_persona_id("Sigrid_") == "sigrid"
    ...
    def test_cry_entity_name(self):     assert normalize_persona_id("NPC_Sigrid_01") == "npc_sigrid_01"
...
class TestBuildQueryBody:
    ...
    def test_persona_id(self):
        p = json.loads(build_query_body("npc_sigrid_01", ""))
        assert p["persona_id"] == "npc_sigrid_01"
...
class TestBuildObservationBody:
    def test_basic(self):
        p = json.loads(build_observation_body("Entity spawned", "Sigrid spawned."))
        assert p["event_type"] == "observation"
        assert p["payload"]["title"] == "Entity spawned"
    ...
    def test_cry_specific(self):
        p = json.loads(build_observation_body("CVar changed", "wyrd_host set to 192.168.1.1"))
        assert "192" in p["payload"]["summary"]
...
class TestBuildFactBody:
    def test_basic(self):
        p = json.loads(build_fact_body("sigrid", "level", "Crysis_Island"))
        assert p["payload"]["value"] == "Crysis_Island"
...
class TestToFacts:
    def test_name(self):
        facts = to_facts("Sigrid", "id-001", None, None)
        assert any(f["key"] == "name" for f in facts)

    def test_entity_id(self):
        facts = to_facts("X", "id-abc", None, None)
        assert any(f["key"] == "entity_id" for f in facts)

    def test_level_included(self):
        facts = to_facts("X", "id", "Crysis_Island", None)
        assert any(f["key"] == "level" for f in facts)
...
class TestParseResponse:
    ...
    def test_long(self):
        body = json.dumps({"response": "X" * 200})
        assert len(parse_response(body)) == 200
